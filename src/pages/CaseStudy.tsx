      ...
      <div className="mx-auto max-w-6xl px-5 py-10 sm:px-8 lg:py-16">
        ...
        <div className="grid gap-12 py-14 lg:grid-cols-[1.3fr_0.7fr]">
          <div className="space-y-12">
            ...
            <section><h2 className="mb-4 text-2xl font-medium">Architecture</h2><p className="mb-6 text-gray-400">{project.architectureSummary}</p><div className="grid gap-3 sm:grid-cols-5">{architecture.map((node, index) => <div key={node.id} className="relative rounded-2xl border border-[#e8b923]/20 bg-white/[0.03] p-4">{index < architecture.length - 1 && <span className="absolute -right-3 top-1/2 hidden text-[#e8b923] sm:block">→</span>}<p className="mb-2 font-mono text-[10px] uppercase tracking-wider text-[#e8b923]">0{index + 1}</p><h3 className="text-sm font-medium text-white">{node.label}</h3>{node.detail && <p className="mt-2 text-xs leading-relaxed text-gray-500">{node.detail}</p>}</div>)}</div></section>
            <section><h2 className="mb-6 text-2xl font-medium">Technical decisions and trade-offs</h2><div className="space-y-4">{decisions.map((item) => <article key={`${item.title}-${item.orderIndex}`} className="rounded-2xl border border-white/10 bg-white/[0.025] p-5"><h3 className="mb-2 font-medium text-white">{item.title}</h3><p className="text-sm leading-7 text-gray-300">{item.decision}</p><p className="mt-3 text-sm leading-7 text-gray-500"><span className="text-[#e8b923]">Trade-off:</span> {item.tradeoff}</p></article>)}</div></section>
        ...
        <section className="border-t border-white/10 py-14"><h2 className="mb-7 text-2xl font-medium">Screenshots and workflow evidence</h2>{media.length ? <div className="grid gap-6 md:grid-cols-2">{media.map((item) => <figure key={`${item.url}-${item.orderIndex}`} className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.025]"><img src={item.url} alt={item.altText} className="aspect-video w-full object-cover" loading="lazy" /><figcaption className="p-4 text-sm leading-relaxed text-gray-400"><span className="mb-1 block font-mono text-[10px] uppercase tracking-wider text-[#e8b923]">{item.kind}</span>{item.caption}</figcaption></figure>)}</div> : <div className="rounded-3xl border border-dashed border-white/15 bg-white/[0.02] p-10 text-center text-sm text-gray-500">Screenshots will be added when verified project media is available.</div>}</section>
